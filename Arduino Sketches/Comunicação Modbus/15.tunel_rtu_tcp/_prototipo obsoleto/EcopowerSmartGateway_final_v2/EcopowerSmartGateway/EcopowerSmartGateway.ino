...
Modbus::ResultCode onRtuFrame(uint8_t* frame, uint8_t len, void* data) {
...
Modbus::ResultCode onTcpFrame(uint8_t* frame, uint8_t len, void* data) {
    if (len > 0 && len < BUFFER_SIZE) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (rtuBufferLen > 0) {
            ...
            if (targetIP.fromString(gatewayConfig.targetIp)) {
                mbTCP.send(targetIP, MODBUS_TCP_PORT, rtuBuffer, rtuBufferLen);
                Debug.println("RTU Frame enviado para TCP.");
            }
            rtuBufferLen = 0; // Limpa o buffer
            return Modbus::ResultCode::SUCCESS;
        }

        // 4.2. Encaminhar de TCP para RTU
        if (tcpBufferLen > 0) {
            mbRTU.send(MODBUS_RTU_SLAVE_ID, tcpBuffer, tcpBufferLen);
