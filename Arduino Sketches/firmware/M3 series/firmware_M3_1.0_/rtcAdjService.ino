...
bool ntpGet(uint32_t &unixOut) {
  ...
  if (t < rtcSvc.UNIX_MIN_2000 || t > rtcSvc.UNIX_MAX_2068) return false;

  unixOut = (uint32_t)t;
  return true;
}

// Callback específico do RTC (não depende de state, usa o ponteiro 'data')
bool rtcCb(Modbus::ResultCode event, uint16_t /*transactionId*/, void* data) {
  auto* svc = static_cast<RtcAdjustService*>(data);
  if (!svc) return true;
  ...
  if (wsClient != 255) {
    ...
    if (svc->modbusOk) {
      ...
      snprintf(buffer, sizeof(buffer), "ERR: SYS_TIME: 0x%02X. %s", event, resultCode(event));
    }
...
bool modbusRequestInverterTime(RtcAdjustService &svc) {

  // Não dispara se já tem transação em andamento
  if (rtu.slave()) return false;

  // Nova tentativa -> zera flags de conclusão
...
// --------------------------------- NTP ---------------------------------------
    ...
    case RtcAdjState::REQUEST_MODBUS:
      ...
      if(modbusRequestInverterTime(svc)) {
