...
bool ProgramCb(Modbus::ResultCode event, uint16_t transactionId, void* data) { // Callback to monitor errors
// Essa função é essencial para o funcionamento e não só para comunicação,
  char buffer[128];

  // cb da verificação de programas
  if (event == Modbus::EX_SUCCESS) {
    ProgSvc.modbusDone = true;
    snprintf(buffer, sizeof(buffer), "LOG: Programa ativo: ", ProgSvc.currentProgram);
  } else {
    ProgSvc.modbusDone = false;
    snprintf(buffer, sizeof(buffer), "ERR: Erro na leitura do registrador ACT_PWR_LIM_VL: 0x%02X. %s", event, resultCode(event));
  }
