...
#define POT_INV 100000
#define NUM_PROGRAMS 6

struct Program {
  uint8_t id;                   // ID do programa (1 a 4)
  uint8_t dayOfWeek;            // Dia da semana (1 - domingo, 6 - sabado)
...
Program programs[NUM_PROGRAMS] PROGMEM = {
  {0x01, 6,  9,  0, 11,  0,  27000, true},
  {0x02, 6, 11,  0, 14,  0,      0, true},
  {0x03, 6, 14,  0, 17,  0,  23000, true},
  {0x04, 6, 17,  0, 18,  0, 100000, true},
  {0x05, 5, 17, 35, 17, 45,   2000, true},
  {0x06, 5, 17, 50, 18,  0,   5000, true},
};

const Program defaultProgram PROGMEM  = {0x00, 0,  0,  0,  0,  0, 100000, true};
...
void checkPrograms(DateTime now) {
  ...
  for (int i = 0; i < NUM_PROGRAMS; i++) {
    ...
    if (!p.enabled || now.dayOfTheWeek() != p.dayOfWeek) continue;
